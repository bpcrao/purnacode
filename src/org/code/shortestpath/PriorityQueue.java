...
/**
 *  @author Owen Astrachan taken from
 *  
 *  http://www.cs.duke.edu/csed/poop/huff/spring05/code/PriorityQueue.java
 *  
...
public class PriorityQueue extends AbstractCollection {
	...
	/**
	 * This is a trivial iterator class that returns elements in the
	 * PriorityQueue ArrayList field one-at-a-time
	 */
	...
	/**
	 * constructs an empty priority queue, when new elements are added their
	 * natural order will be used to determine which is minimal. This means
	 * elements that are added must implement the <code>Comparable</code>
	 * interface and must be <em>mutually comparable</em>, i.e.,
	 * <code>e1.compareTo(e2)</code> must not throw a
	 * <code>CastCastException</code> for any two elements <code>e1</code> and
	 * <code>e2</code> in the priority queue.
	 */
	...
	/**
	 * constructs an empty priority queue, when new elements are added the
	 * <code>Comparator comp</code> determines which is smaller.
	 * 
	 * @param comp
	 *            is the <code>Comparator</code> used in determining order
	 */
	...
	/**
	 * all elements in coll are added to the priority queue. The complexity is
	 * O(n) where <code>coll.size() == n</code>
	 * 
	 * @param coll
	 *            is a collection of mutually comparable elements
	 */
	...
	/**
	 * A new element <code>o</code> is added to the priority queue in O(log n)
	 * time where n is the size of the priority queue.
	 * <P>
	 * The return value should be ignored, a boolean value must be returned
	 * because of the requirements of the <code>Collection</code> interface.
	 * 
	 * @param o
	 *            is the (Comparable) object added to the priority queue
	 * @return true
	 */
	...
	/**
	 * The smallest/minimal element is removed and returned in O(log n) time
	 ...
	 * 
	 * @param vroot
	 *            is the index at which re-heaping occurs
	...
	private void heapify(int vroot) {
		...
		while (2 * k <= mySize) {
			child = 2 * k;
			if (child < mySize
					&& myComp.compare(myList.get(child), myList.get(child + 1)) > 0) {
				child++;
			}
	...
	/**
	 * simple test harnass that inserts all arguments into a priority queue and
	 * then removes them one at a time and prints them one per line as they are
	 * removed thus effectively sorting in O(n log n) time
	 */
	...
	public void update(Vertex vertex) {
		...
		System.out.println("removed "+this.remove(vertex));
		this.add(vertex);
